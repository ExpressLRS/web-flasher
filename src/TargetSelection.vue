...
function updateVendors() {
    fetch(`/assets/${store.firmware}/${store.version}/hardware/targets.json`).then(r => r.json()).then(r => {
        ...
        for (const [k, v] of Object.entries(hardware)) {
            // should filter out vendors that don't have any of the given targetType
...
<template>
    ...
    <VRow>
        <VCol cols="12">
            ...
            <VSelect :items="vendors" v-model="store.vendor" density="comfortable" label="Hardware Vendor" :disabled="!store.version"/>
            <VSelect :items="radios" v-model="store.radio" density="comfortable" label="Radio Frequency" :disabled="!store.vendor"/>
